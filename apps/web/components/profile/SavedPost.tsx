...
import { Post,type SavedPost } from '@repo/trpc/post'
...
const SavedPost = ({user}:{user:UserProfile}) => {
    ...
    const trpc = useTRPC()
      const {
          ...
          isLoading,
          isFetching,
        } = useInfiniteQuery(trpc.post.getSavedPosts.infiniteQueryOptions(
          {
            limit:3,
            cursor:undefined,
          },
        ...
        <div className='grid grid-cols-2 gap-2 mt-6  w-full'>
            {
              savedPosts.map((savedPost,idx)=>(
                  <div  key={idx} className={cn('rounded-lg relative group overflow-hidden', idx===0 && 'col-span-2')}>
                    <div className='w-full h-80 rounded-lg relative ' style={{ aspectRatio: savedPost.post.medias[0]?.aspect || '1/1' }}>

                      <CldImage src={getImageUrl(savedPost.post?.medias[0]!)} alt={savedPost.post?.medias[0]?.altText || 'post media'} fill className='object-cover group-hover:scale-110 transition-transform duration-300 ' preserveTransformations />          
                    </div>
                      <div onClick={()=>{
                              setActivePost(savedPost.post)
                              setIsOpened(true)
                            ...
                            <div className='flex group-hover:opacity-100 opacity-0 transition-opacity duration-300 items-center gap-2 cursor-pointer font-bold'>
                                ...
                                <span className='text-white '>{savedPost.post.likesCount}</span>
                            </div>
                            <div  className='flex group-hover:opacity-100 opacity-0 transition-opacity duration-300 items-center gap-2 cursor-pointer font-bold'>
                                <FaMessage className='text-white text-xl' />
                                <span className='text-white '>{savedPost.post.commentsCount}</span>
                            </div>
                          
                      </div>
                      {savedPost.post.medias.length > 1 && (
                        <div className="absolute top-2 right-2 rounded-full p-1">
                          <div className="flex gap-0.5">
                            {savedPost.post.medias.slice(0, 3).map((_, i) => (
